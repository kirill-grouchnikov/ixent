...
/**
 * The aggregate coarsener, based on <i>"Fast Multiscale Image Segmentation"</i>
 * by Sharon, Brandt and Basri in 2000 paper. Achieves higher degree of
 * coarsening than the basic method explained in the above paper
 *
 * @author Kirill Grouchnikov
 */
public class AggregateCoarsener extends Coarsener {
    private Logger logger;

    public AggregateCoarsener() {
        this.logger =
                Logger.getLogger(
                        AggregateCoarsener.class.getPackage().getName());
    }
    ...
    public GraphInfo getNextGraph(int origSize) {
        ...
        double currFactor = (nodeCount < 500) ?
                0.15 : 0.25 * Math.exp(-0.1 * this.prevGraphInfo.getIndex());
        while (true) {
            // 2. select biggest segment
            int biggestSegmentIndex = orderedNodes[startingNodeIndex];
            ...
            // 3. for each biggest unprocessed segment:
            for (int currIndex = 0; currIndex < nodeCount; currIndex++) {
                if (currIndex == startingNodeIndex) {
                    continue;
                }
                ...
                for (int neighIndex = 0;
                    ...
                    if (nodeCount < 500) {
                        ...
                        if (capturers[currNeighbourIndex] == null) {
                            continue;
                        }
                    }
                ...
                // check if strongly attached. The criterion - maximal weighted
                // to selected node is higher than average weight to any neighbour
                int neighbourCount = currNode.getNeighbourCount();
//                if (maximalWeightToSelected > (totalWeight/(0.1+neighbourCount))) {
                if (maximalWeightToSelected > (currFactor * totalWeight)) {
                    //    b. if strongly coupled - finish processing
                    ...
                    //    c. if loosely coupled - add to selected
                    newNode =
                            new NodeInfo(selectedNodes.size(),
                                    this.prevGraphInfo.getNode(currNodeIndex)
                            .getVolume());
            ...
            if (selectedNodes.size() == nodeCount) {
                startingNodeIndex++;
                if (startingNodeIndex == nodeCount) {
                    break;
                }
                // no coarsening made. Try starting from different node
                this.logger.fine("No coarsening made. Go to the next node");
                selectedNodes.clear();
                for (int currIndex = 0; currIndex < nodeCount; currIndex++) {
                    isSelected[currIndex] = false;
                    capturers[currIndex] = null;
                }
                currFactor -= 0.02;
            }
            else {
